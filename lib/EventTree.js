...
class EventTree {
  ...
  on(path, listener) {
    ...
    this.node = this.tree.root.clone();
    EventTree._expandNodes(this.node);
  }

  /**
   * @param {Node} node
   */
  static _expandNodes(node) {
    const wildNode = node.getChild(WILDCARD);
    if (wildNode !== undefined) {
      node.children.forEach((child) => {
        if (child.name !== WILDCARD) {
          child.addListeners(wildNode.listeners);
          EventTree._mergeNodes(child, wildNode);
        }
      });
    }

    node.children.forEach(child => EventTree._expandNodes(child));
  }
   ...
   * @param {Node} source
   */
  static _mergeNodes(target, source) {
    source.children.forEach((sourceChild) => {
      ...
      EventTree._mergeNodes(targetChild, sourceChild);
    });
  }
