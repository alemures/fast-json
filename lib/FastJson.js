...
class FastJson {
  ...
  constructor(options) {
    ...
    this._skipBlockOffset = 0;
    this._skipBlockDepth = 1;
    this._parseStringOffset = 0;
    this._parsePrimitiveOffset = 0;
  ...
  static _skipBlock(data, index, openChar, closeChar) {
    ...
    if (this._skipBlockOffset) {
      index = this._skipBlockOffset - 1;
      blockDepth = this._skipBlockDepth;
      this._skipBlockOffset = 0;
    }
    ...
    for (let i = index + 1; i < data.length; i++) {
      switch (FastJson._get(data, i)) {
        case QUOTE:
          const temp = i;
          i = FastJson._parseString(data, i);
          if (i === -1) {
            this._skipBlockOffset = temp;
            this._skipBlockDepth = blockDepth;
            return -1;
          }
    ...
    this._skipBlockOffset = data.length;
    this._skipBlockDepth = blockDepth;
  ...
  static _parseString(data, index) {
    if (this._parseStringOffset) {
      index = this._parseStringOffset - 1;
      this._parseStringOffset = 0;
    }
    ...
    this._parseStringOffset = data.length;
  ...
  static _parsePrimitive(data, index) {
    if (this._parsePrimitiveOffset) {
      index = this._parsePrimitiveOffset;
      this._parsePrimitiveOffset = 0;
    }
    ...
    this._parsePrimitiveOffset = data.length;
