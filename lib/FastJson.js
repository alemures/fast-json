...
class FastJson {
  ...
  _onQuote(data, index) {
    const frame = this._stack[this._level];
    const str = { start: index + 1, end: this._parseString(data, index) };
    let path;
  ...
  _onPrimitive(data, index) {
    const frame = this._stack[this._level];
    const primEnd = this._parsePrimitive(data, index);
    let path;
  ...
  _skipBlock(data, index, openChar, closeChar) {
    ...
    for (let i = index + 1; ;i++) {
      switch (this._get(data, i)) {
        case QUOTE:
          strEnd = this._parseString(data, i);
          i += strEnd - i;
          break;
        case openChar:
          blockDepth++;
  ...
  _resolvePathForPrimitiveObject(data) {
    if (this._level === 0) {
      return `${SEP}${this._toString(data, this._lastString.start, this._lastString.end)}`;
    }
