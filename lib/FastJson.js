...
class FastJson {
  ...
  _parseString(data, index) {
    index++;
    while (this._get(data, index) !== QUOTE || (this._get(data, index - 1) === BACKSLASH &&
        this._get(data, index - 2) !== BACKSLASH)) {
      index++;
    }
  ...
  _parsePrimitive(data, index) {
    while (this._get(data, index) !== CLOSE_BRACKET && this._get(data, index) !== CLOSE_BRACE &&
        this._get(data, index) !== COMMA) {
      index++;
    }
