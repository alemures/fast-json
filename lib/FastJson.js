...
class FastJson {
  ...
  _parseString(data, index) {
    index++;
    while (data.charCodeAt(index) !== QUOTE || (data.charCodeAt(index - 1) === BACKSLASH && data.charCodeAt(index - 2) !== BACKSLASH)) {
      index++;
    }
  ...
  _parsePrimitive(data, index) {
    while (data.charCodeAt(index) !== CLOSE_BRACKET && data.charCodeAt(index) !== CLOSE_BRACE && data.charCodeAt(index) !== COMMA) {
      index++;
    }
