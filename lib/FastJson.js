...
class FastJson {
  ...
  _onOpenBrace(data, index) {
    ...
    const parentFrame = this._stack[this._level - 1];
    const path = this._resolvePath(data, parentFrame);
   ...
   * @returns {String}
   */
  _resolveParentKey(data, parentFrame) {
    if (parentFrame.type === TYPE_ARRAY) {
      return parentFrame.index;
    }
   ...
   * @returns {String}
   */
  _resolvePath(data, parentFrame) {
    if (this._level === 0) {
      return SEP;
    } else if (this._level === 1) {
      return `${SEP}${this._resolveParentKey(data, parentFrame)}`;
    }

    return `${parentFrame.path}${SEP}${this._resolveParentKey(data, parentFrame)}`;
  }
   ...
   * @returns {String}
   */
  _resolvePathForPrimitiveArray(frame) {
    if (this._level === 0) {
      return `${SEP}${frame.index}`;
    }

    return `${frame.path}${SEP}${frame.index}`;
  }
